...
namespace Sprint2
{
    public class LevelLoaderFactory
    {
        ...
        public ILevel LoadContents(string xmlString, int roomNum)
        {
            ...
            while (reader.Read())
            {
                ...
                if (reader.LocalName == "room" && int.Parse(reader.GetAttribute("id")) == roomNum)
                {
                    ...
                    inner.ReadToFollowing("Artifact");
                    while (inner.Read())
                    {
        ...
        private static void CreateGameObjects(ILevel level, string name, string type, int xPos, int yPos, int width, int height)
        {
            ...
            {
                case "Player":
                    CreatePlayer(level, type, xPos, yPos);
                    break;
                case "Enemy":
        ...
        private static void CreatePlayer(ILevel level, string type, int xPos, int yPos)
        {
            switch (type)
            {
                case "Link":
                    level.Link = new Link(xPos, yPos, level);
                    break;
                default:
                    break;
            }
        ...
        private static void CreateEnemy(ILevel level, string type, int xPos, int yPos)
        {
            ...
            {
                case "Aquamentus":
                    level.Enemies.Add(new Aquamentus(xPos, yPos));
                    break;
                case "Gel":
        ...
        private static void CreateItem(ILevel level, string type, int xPos, int yPos)
        {
            ...
            {
                case "Arrow":
                    level.Items.Add(new Arrow(xPos, yPos));
                    break;
                case "BlueRupy":
        ...
        private static void CreateArtifact(ILevel level, string type, int xPos, int yPos, int width, int height)
        {
            switch (type)
            {
                ...
                case "Water":
                    ...
                    break;
                case "Door":
                    level.Doors.Add(new Door(xPos, yPos, width, height));
                    break;
                default:
                    break;
            }
        ...
        private static void CreateProjectile(ILevel level, string type, int xPos, int yPos)
        {
            switch (type)
            {
                case "Bomb":
                    ...
                    break;
                case "Boomerang":
                    level.Projectiles.Add(new Boomerang(xPos, yPos));
                    break;
                default:
                    break;
            }
